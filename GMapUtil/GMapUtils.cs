...
namespace GMapUtil
{
    ...
    public static class GMapUtils
    {
        ...
        /// <param name="X"></param>
        /// <returns></returns>
        public static bool IsPointInPolygon(List<PointLatLng> points, PointLatLng Punto)
        {
            ...
            for (int i = 0; i < points.Count - 1; i++)
            {
                ...
                var m = (points[j].Lat - points[i].Lat) / (points[j].Lng - points[i].Lng);
                //m of the ray is = 0

                //q value of the line
                var qSegment = (-m * points[i].Lng) + points[i].Lat;
                //q value of the ray
                var qPointRay = Punto.Lat;


                //Get bigger and smaller x and y values to determine if the point is inside the segment
                double biggerX = (points[i].Lng - points[j].Lng) >= 0 ? points[i].Lng : points[j].Lng;
                double biggerY = (points[i].Lat - points[j].Lat) >= 0 ? points[i].Lat : points[j].Lat;
                double smallerX = (points[i].Lng - points[j].Lng) >= 0 ? points[j].Lng : points[i].Lng;
                double smallerY = (points[i].Lat - points[j].Lat) >= 0 ? points[j].Lat : points[i].Lat;

                //Check if point is on the line created by the segment
                if (m * Punto.Lng + qSegment - Punto.Lat == 0)
                {
                    //Check if the point is in the segment
                    if (Punto.Lng <= biggerX && Punto.Lng >= smallerX && Punto.Lat <= biggerY && Punto.Lat >= smallerY)
                    {
                ...
                PointLatLng intersectionPoint = new PointLatLng { Lng = intersectionX, Lat = intersectionY };

                if (intersectionPoint.Lng <= biggerX && intersectionPoint.Lng >= smallerX && intersectionPoint.Lat <= biggerY && intersectionPoint.Lat >= smallerY && intersectionPoint.Lng >= Punto.Lng)
                {
