...
namespace GMapPositionFix
{
    ...
    public static class CoordinateTransform
    {
        ...
        public static void LonLat2Mercator(double lngWgs, double latWgs, out double lng, out double lat)
        {
            double x = lngWgs * 20037508.34 / 180;
            double y = Math.Log(Math.Tan((90 + latWgs) * Math.PI / 360)) / (Math.PI / 180);
            y = y * 20037508.34 / 180;
            lng = x;
            lat = y;
        }
        //墨卡托转经纬度
        public static void Mercator2lonLat(double lng, double lat, out double lngWgs, out double latWgs)
        {
