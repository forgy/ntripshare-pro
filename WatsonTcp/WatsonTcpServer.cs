...
namespace WatsonTcp
{
    ...
    public class WatsonTcpServer : IDisposable
    {
        ...
        public void Start()
        {
            ...
            if (_Mode == Mode.Tcp)
            {
                Logger?.Invoke("[TcpServer] Starting on " + _ListenerIp + ":" + _ListenerPort);
            }
            else if (_Mode == Mode.Ssl)
            {
                Logger?.Invoke("[TcpServer] Starting with SSL on " + _ListenerIp + ":" + _ListenerPort);
            }
            else
            {
        ...
        public Task StartAsync()
        {
            ...
            if (_Mode == Mode.Tcp)
            {
                Logger?.Invoke("[TcpServer] 开始监听 " + _ListenerIp + ":" + _ListenerPort); 
            }
            else if (_Mode == Mode.Ssl)
            {
                Logger?.Invoke("[TcpServer] Starting with SSL on " + _ListenerIp + ":" + _ListenerPort);
        ...
        public bool Send(string ipPort, Dictionary<object, object> metadata, byte[] data)
        {
            ...
            {
                Logger?.Invoke("[TcpServer] Unable to find client " + ipPort);
                return false;
            }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                Logger?.Invoke("[TcpServer] Dispose complete");
            }
        }
        ...
        private async Task AcceptConnections()
        {
            ...
            while (!_Token.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    {
                        Logger?.Invoke("[TcpServer] Rejecting connection from " + clientIp + " (not permitted)");
                        tcp.Close();
                        continue;
                    }
                    ...
                    {
                        Logger?.Invoke("[TcpServer] Maximum connections " + _MaxConnections + " met or exceeded (currently " + _Connections + " connections), pausing listener");
                        _IsListening = false;
                        _Listener.Stop();
                    }
                    ...
                    Logger?.Invoke("[TcpServer] 接受客户端连接 " + client.IpPort);

                    #endregion 
                }
                catch (ObjectDisposedException)
                {
                    Logger?.Invoke("[TcpServer] Disposal detected, closing connection listener");
                }
                catch (Exception e)
                {
                    Logger?.Invoke("[TcpServer] Exception for " + ipPort + ": " + e.Message);
                }
            }
        }
        ...
        private async Task<bool> StartTls(ClientMetadata client)
        {
            ...
            { 
                ...
                {
                    Logger?.Invoke("[TcpServer] Stream from " + client.IpPort + " not encrypted");
                    client.Dispose();
                    _Connections--;
                    return false;
                }
                ...
                {
                    Logger?.Invoke("[TcpServer] Stream from " + client.IpPort + " not authenticated");
                    client.Dispose();
                    _Connections--;
                    return false;
                }
                ...
                {
                    Logger?.Invoke("[TcpServer] Stream from " + client.IpPort + " failed mutual authentication");
                    client.Dispose();
                    _Connections--;
                    return false;
                }
            ...
            catch (IOException ex)
            {
                ...
                switch (ex.Message)
                {
                    ...
                    case "Unable to read data from the transport connection: An existing connection was forcibly closed by the remote host.":
                        Logger?.Invoke("[TcpServer] Connection closed by " + client.IpPort + " during SSL negotiation");
                        break;

                    case "The handshake failed due to an unexpected packet format.":
                        Logger?.Invoke("[TcpServer] Disconnected " + client.IpPort + " due to invalid handshake");
                        break;

                    default:
                        Logger?.Invoke(
                            "[TcpServer] Disconnected " + client.IpPort + " due to TLS exception: " +
                            Environment.NewLine +
            ...
            {
                Logger?.Invoke("[TcpServer] Exception on " + client.IpPort + " during TLS negotiation: " + Environment.NewLine + ex.ToString());
                client.Dispose();
                _Connections--;
                return false;
            }
        ...
        private void FinalizeConnection(ClientMetadata client)
        {
            ...
            {
                Logger?.Invoke("[TcpServer] Soliciting authentication material from " + client.IpPort);
            ...
            Logger?.Invoke("[TcpServer] 开始接收数据 " + client.IpPort);
        ...
        private bool IsConnected(ClientMetadata client)
        {
            if (client.TcpClient.Connected)
            {
                ...
                {
                    Logger?.Invoke("[TcpServer] Exception while testing connection to " + client.IpPort + " using send: " + e.Message);
                    success = false;
                }
                finally
                {
                ...
                {
                    Logger?.Invoke("[TcpServer] Exception while testing connection to " + client.IpPort + " using poll/peek: " + e.Message);;
                    return false;
                }
                finally
                {
        ...
        { 
            while (true)
            {
                try
                {
                    ...
                    if (!buildSuccess)
                    {
                        Logger?.Invoke("[TcpServer] Message build failed due to disconnect for client " + client.IpPort);
                        break;
                    }
                    ...
                    if (!String.IsNullOrEmpty(PresharedKey))
                    {
                        ...
                        {
                            Logger?.Invoke("[TcpServer] Message received from unauthenticated endpoint " + client.IpPort);
                            ...
                            if (msg.Status == MessageStatus.AuthRequested)
                            {
                                ...
                                if (msg.PresharedKey != null && msg.PresharedKey.Length > 0)
                                {
                                    ...
                                    {
                                        Logger?.Invoke("[TcpServer] Accepted authentication for " + client.IpPort);
                                    ...
                                    {
                                        Logger?.Invoke("[TcpServer] Declined authentication for " + client.IpPort);
                                ...
                                {
                                    Logger?.Invoke("[TcpServer] No authentication material for " + client.IpPort);
                    ...
                    {
                        Logger?.Invoke("[TcpServer] Received notification of disconnection from " + client.IpPort);
                        break;
                    }
                    else if (msg.Status == MessageStatus.Removed)
                    {
                        Logger?.Invoke("[TcpServer] Sent notification of removal to " + client.IpPort);
                        break;
                    }
                    ...
                    if (msg.SyncRequest)
                    {
                        ...
                        if (SyncRequestReceived != null)
                        {
                            ...
                            {
                                Logger?.Invoke("[TcpServer] Expired synchronous request received and discarded from " + client.IpPort);
                            }
                        } 
                    }
                    else if (msg.SyncResponse)
                    {
                        ...
                        {
                            Logger?.Invoke("[TcpServer] Expired synchronous response received and discarded from " + client.IpPort);
                        }
                    }
                    else
                    {
                        ...
                        {
                            Logger?.Invoke("[TcpServer] Event handler not set for either MessageReceived or StreamReceived");
                            break;
                        }
                    }
                ...
                {
                    Logger?.Invoke("[TcpServer] Client " + client.IpPort + " disconnected due to disposal");
                    break;
                }
                catch (OperationCanceledException)
                {
                    Logger?.Invoke("[TcpServer] Cancellation requested");
                    break;
                }
                catch (Exception e)
                { 
                    Logger?.Invoke(
                        "[TcpServer] Data receiver exception for " + client.IpPort + ":" +
                        Environment.NewLine +
            ...
            Logger?.Invoke("[TcpServer] Data receiver terminated for " + client.IpPort);
            ...
            Logger?.Invoke("[TcpServer] Disposing data receiver for " + client.IpPort);
            client.Dispose();  
        }
        ...
        private bool SendInternal(ClientMetadata client, WatsonMessage msg, long contentLength, Stream stream)
        {
            ...
            {
                Logger?.Invoke("[TcpServer] Failed to write message to " + client.IpPort + " due to exception: " + e.Message);
                return false;
            }
            finally
            {
        ...
        { 
            ...
            { 
                if (curr.Value < idleTimestamp)
                {
                    ...
                    Logger?.Invoke("[TcpServer] Disconnecting client " + curr.Key + " due to idle timeout");
            ...
            while (_TokenSource != null && !_TokenSource.IsCancellationRequested)
            {
                ...
                lock (_SyncResponseLock)
                {
                        ...
                        ))
                    {
                        ...
                        {
                            Logger?.Invoke("[TcpServer] MonitorForExpiredSyncResponses expiring response " + curr.Key.ToString());
